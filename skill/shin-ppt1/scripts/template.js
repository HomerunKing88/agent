...
const F  = "맑은 고딕";     // 제목·요약박스를 뺀 전부
const FH = "HY헤드라인M";   // 페이지 제목과 상단 요약박스
const FOOT_BASE = 8.14;     // 각주 바닥 기준선
...
function box(s, o) {
  if (!RADIUS) { s.addShape("rect", o); return; }
  const r = Math.min(RADIUS, Math.min(o.w, o.h) * 0.22);
  s.addShape("roundRect", Object.assign({}, o, { rectRadius: r }));
}
...
const TS = {
  title:     19,    // 페이지 제목 (HY헤드라인M)
  summary:   15,    // 상단 요약박스
  sub:       13.5,  // 소제목
  subDesc:   11,    // 소제목 옆 보조 설명, 우측 기준일 표기
  table:     12.5,  // 표 본문 기본
  tableMin:  11.5,  // 표 본문 하한. 이 아래로 내리지 않는다
  bigValue:  22,    // 분기 도식 머리의 대표 숫자
  cardTitle: 14,    // 도식 카드 제목
  cardBody:  12,    // 도식 카드 설명, 본문 강조 박스
  layer:     13,    // 적층·계층·단계 라벨
  bullet:    12,    // 불릿 본문
  gridHead:  12,    // 격자·레인·간트 열 머리
  gridCell:  11.5,  // 격자·레인 칸
  axis:      11,    // 축 라벨, 눈금 이름
  legend:    10.5,  // 범례, 캡션
  value:     10,    // 막대·꺾은선 위 수치
  foot:      9      // 각주
};
...
const THEMES = {
  ...
  // 무채색 전용. accent가 ink와 같아 색으로는 아무것도 강조되지 않는다
  mono: {
  ...
  // 크림 지면. 표에 cardBg를 켜면 종이 위 카드처럼 읽힌다
  paper: {
  ...
  // 다계열 수치. series 순서를 자료 전체에서 고정한다
  dense: {
...
function ruleThick(s, y, x = MX, w = CW) { s.addShape("rect", { x, y, w, h: 0.028, fill: { color: C.ink } }); }
function ruleThin(s, y, x = MX, w = CW) { s.addShape("rect", { x, y, w, h: 0.008, fill: { color: C.rule } }); }

// 눈썹 라벨. 제목 위 분류 표기(자간을 벌린 작은 글씨). 부서명은 넣지 않는다
function eyebrow(s, text, x = MX, y = 0.24) {
  s.addText(text, { x, y, w: CW - 2.6, h: 0.20, fontFace: F, fontSize: TS.legend - 0.5,
...
function header(s, title, opts = {}) {
  ...
  s.addText(title, { x: MX, y: 0.46, w: CW, h: 0.40, fontFace: FH, fontSize: TS.title, bold: true, color: C.ink, margin: 0, valign: "middle" });
  ruleThick(s, 0.90);
}
...
function summary(s, l1, l2, y = 1.06, h = 0.76) {
  ...
  s.addText(runs, { x: MX + 0.18, y, w: CW - 0.36, h, fontFace: FH, align: "center", valign: "middle", margin: 0, lineSpacingMultiple: 1.20 });
  return y + h;
}
...
function msgBox(s, y, h, text, x = MX, w = CW) {
  ...
  s.addText(text, { x: x + 0.22, y, w: w - 0.44, h, fontFace: F, fontSize: TS.cardBody, color: C.msgInk, valign: "middle", margin: 0, lineSpacingMultiple: 1.22 });
  return y + h;
}

// 소제목. accent 세로바 + ink 소제목. 색 면적은 세로바 0.055in 뿐
function sub(s, x, y, label, desc, descW = 3.4) {
  s.addShape("rect", { x, y: y + 0.02, w: 0.06, h: 0.28, fill: { color: C.accent } });
  s.addText(label, { x: x + 0.16, y, w: 3.4, h: 0.32, fontFace: F, fontSize: TS.sub, bold: true, color: C.ink, margin: 0, valign: "middle" });
  if (desc) s.addText(desc, { x: x + 0.16, y, w: descW, h: 0.32, fontFace: F, fontSize: TS.subDesc, color: C.mute, align: "right", margin: 0, valign: "middle" });
}
...
function openTable(s, rows, opts) {
  ...
  if (opts.cardBg) s.addShape("rect", { x, y, w, h: total, fill: { color: C.card } });
  (opts.washCols || []).forEach(ci => {
    const cx = x + colW.slice(0, ci).reduce((a, b) => a + b, 0);
    s.addShape("rect", { x: cx, y: y + rowH[0], w: colW[ci], h: total - rowH[0], fill: { color: C.wash } });
  });
  s.addTable(rows, { x, y, w, colW, rowH, border: { type: "none" }, margin: opts.margin || [3, 8, 3, 8], valign: "middle" });
...
function bullets(s, items, x, y, w, fs = TS.bullet, step = 0.40) {
  items.forEach((runs, i) => {
    ...
    if (!concl) s.addShape("rect", { x: x + 0.01, y: yy + 0.085, w: 0.055, h: 0.055, fill: { color: C.accent } });
    s.addText(runs.map(r => ({ text: r.t, options: { fontSize: fs, color: r.c || C.body, bold: !!r.b } })),
      { x: x + 0.16, y: yy, w: w - 0.16, h: step, fontFace: F, margin: 0, valign: "top", lineSpacingMultiple: 1.18 });
  });
}
...
function bars(s, px, pw, base, maxH, vmax, labels, vals, hi) {
  ...
  vals.forEach((v, i) => {
    ...
    const h = v / vmax * maxH;
    s.addShape("rect", { x: cx, y: base - h, w: bw, h, fill: { color: on ? C.accent : C.rule } });
    s.addText(String(v), { x: cx - 0.22, y: base - h - 0.24, w: bw + 0.44, h: 0.22, fontFace: F, fontSize: TS.value, bold: on, color: on ? C.accent : C.body, align: "center", margin: 0 });
    s.addText(labels[i], { x: x0 + i * slot - 0.06, y: base + 0.06, w: slot + 0.12, h: 0.34, fontFace: F, fontSize: TS.axis, color: on ? C.ink : C.mute, align: "center", margin: 0, valign: "top", lineSpacingMultiple: 1.0 });
  });
...
function barsGroup(s, x, w, base, maxH, vmax, labels, series, opts = {}) {
  ...
  series.forEach((sr, si) => {
    s.addShape("rect", { x: lx, y: ly + 0.07, w: 0.16, h: 0.10, fill: { color: C.series[si] } });
    s.addText(sr.name, { x: lx + 0.24, y: ly, w: 1.22, h: 0.26, fontFace: F, fontSize: TS.legend, color: C.body, margin: 0, valign: "middle" });
    lx += 1.24;
  });
  ...
  labels.forEach((lb, i) => {
    ...
    series.forEach((sr, si) => {
      ...
      s.addShape("rect", { x: gx + si * bw, y: base - bh, w: bw - 0.03, h: bh, fill: { color: C.series[si] } });
      if (opts.values !== false) {
        s.addText(String(v), { x: gx + si * bw - 0.20, y: base - bh - 0.22, w: bw + 0.37, h: 0.20, fontFace: F, fontSize: TS.value, color: C.mute, align: "center", margin: 0 });
      }
    });
    s.addText(lb, { x: x + i * slot, y: base + 0.06, w: slot, h: 0.30, fontFace: F, fontSize: TS.axis, color: C.ink, align: "center", margin: 0, valign: "top" });
  });
  ruleThin(s, base, x, w);
  return base + 0.36;
}
...
function caption(s, x, y, w, text) {
  s.addText(text, { x, y, w, h: 0.28, fontFace: F, fontSize: TS.legend, color: C.mute, margin: 0 });
}
...
function footer(s, notes, y, opts = {}) {
  ...
  s.addText(notes.map((t, i) => ({ text: t, options: { breakLine: i < notes.length - 1 } })),
    { x: MX, y, w: CW - pw, h: 0.15 * notes.length + 0.12, fontFace: F, fontSize: TS.foot, color: C.mute, margin: 0, valign: "top", lineSpacingMultiple: 1.14 });
  // 페이지 표기는 각주 줄 오른쪽 끝. 제목 옆이 아니라 여기다
  if (opts.page) {
    s.addText(String(opts.page), { x: MX + CW - pw, y, w: pw, h: 0.20, fontFace: F, fontSize: TS.foot,
...
function icon(s, x, y, sz, kind, color) {
  ...
  const ln = (x1, y1, w, h) => s.addShape("line", { x: x1, y: y1, w, h, line: L });
  const rc = (x1, y1, w, h) => s.addShape("rect", { x: x1, y: y1, w, h, fill: { type: "none" }, line: L });
  const el = (x1, y1, w, h) => s.addShape("ellipse", { x: x1, y: y1, w, h, fill: { type: "none" }, line: L });
  const u = sz;
  switch (kind) {
    case "bank":
      ln(x, y + u * 0.30, u, 0);
    ...
    case "people":
      ...
      s.addShape("roundRect", { x: x + u * 0.16, y: y + u * 0.52, w: u * 0.66, h: u * 0.34, rectRadius: 0.04, fill: { type: "none" }, line: L });
      break;
    case "check":
      ...
      break;
    case "warn":
      s.addShape("triangle", { x: x + u * 0.04, y: y + u * 0.10, w: u * 0.92, h: u * 0.78, fill: { type: "none" }, line: L });
...
function flow(s, x, y, w, h, steps, opts = {}) {
  ...
  steps.forEach((st, i) => {
    ...
    if (opts.bare) {
      ...
      s.addShape("rect", { x: bx, y, w: bw, h: on ? 0.03 : 0.012, fill: { color: on ? C.accent : C.rule } });
      if (on) s.addShape("rect", { x: bx, y: y + 0.03, w: bw, h: h - 0.03, fill: { color: C.wash } });
    } else {
    ...
    s.addText(st.title, { x: bx + 0.08, y: y + 0.66, w: bw - 0.16, h: 0.32, fontFace: F, fontSize: TS.cardTitle, bold: true, color: C.ink, align: "center", valign: "middle", margin: 0 });
    if (st.desc) s.addText(st.desc, { x: bx + 0.10, y: y + 1.02, w: bw - 0.20, h: h - 1.12, fontFace: F, fontSize: TS.cardBody, color: C.body, align: "center", valign: "top", margin: 0, lineSpacingMultiple: 1.14 });
    if (i < n - 1) {
      s.addShape("rightArrow", { x: bx + bw + 0.09, y: y + h * 0.30, w: 0.24, h: 0.20, fill: { color: C.rule } });
    }
  });
  if (opts.loop) {
    const by = y + h + 0.26, x1 = x + w - bw / 2, x2 = x + bw / 2;
    s.addShape("line", { x: x1, y: y + h, w: 0, h: by - (y + h), line: { color: C.accent, width: 1, dashType: "dash" } });
    s.addShape("line", { x: x2, y: by, w: x1 - x2, h: 0, line: { color: C.accent, width: 1, dashType: "dash" } });
    s.addShape("upArrow", { x: x2 - 0.09, y: by - 0.22, w: 0.18, h: 0.22, fill: { color: C.accent } });
    if (opts.loopLabel) {
      s.addShape("rect", { x: x2 + 0.24, y: by - 0.19, w: x1 - x2 - 0.48, h: 0.18, fill: { color: C.page } });
      s.addText(opts.loopLabel, { x: x2 + 0.20, y: by - 0.20, w: x1 - x2 - 0.40, h: 0.20, fontFace: F, fontSize: TS.axis, bold: true, color: C.accent, align: "center", valign: "middle", margin: 0 });
    }
  }
...
function stack(s, x, y, w, layers, hi, opts = {}) {
  ...
  layers.forEach((L, i) => {
    ...
    s.addText(L.label, { x: x + 0.08, y: ly, w: w * 0.46 - 0.16, h: lh - 0.06, fontFace: F, fontSize: TS.layer, bold: on, color: on ? C.accent : C.ink, align: "center", valign: "middle", margin: 0 });
    if (L.note) s.addText(L.note, { x: x + w * 0.50, y: ly, w: w * 0.50, h: lh - 0.06, fontFace: F, fontSize: TS.cardBody, color: C.body, valign: "middle", margin: 0 });
  });
...
function branch(s, x, y, w, h, head, items, opts = {}) {
  ...
  s.addText(head.title, { x: x + 0.10, y: y + h * 0.13, w: hw - 0.20, h: 0.34, fontFace: F, fontSize: TS.layer, color: C.body, align: "center", valign: "middle", margin: 0 });
  s.addText(head.value, { x: x + 0.10, y: y + h * 0.37, w: hw - 0.20, h: 0.54, fontFace: F, fontSize: TS.bigValue, bold: true, color: C.accent, align: "center", valign: "middle", margin: 0 });
  if (head.note) s.addText(head.note, { x: x + 0.10, y: y + h * 0.74, w: hw - 0.20, h: 0.30, fontFace: F, fontSize: TS.axis, color: C.mute, align: "center", valign: "middle", margin: 0 });

  const ix = x + hw + gap, iw = w - hw - gap, n = items.length;
  const ih = (h - 0.14 * (n - 1)) / n;
  const spine = x + hw + gap * 0.46;
  s.addShape("line", { x: x + hw, y: y + h / 2, w: gap * 0.46, h: 0, line: { color: C.rule, width: 1 } });
  s.addShape("line", { x: spine, y: y + ih / 2, w: 0, h: h - ih, line: { color: C.rule, width: 1 } });
  items.forEach((it, i) => {
    const iy = y + i * (ih + 0.14);
    s.addShape("line", { x: spine, y: iy + ih / 2, w: ix - spine, h: 0, line: { color: C.rule, width: 1 } });
    box(s, { x: ix, y: iy, w: iw, h: ih, fill: { color: C.card }, line: { color: C.rule, width: 0.75 } });
    s.addText(it.label, { x: ix + 0.16, y: iy, w: iw * 0.40, h: ih, fontFace: F, fontSize: TS.layer, bold: true, color: C.ink, valign: "middle", margin: 0 });
    if (it.note) s.addText(it.note, { x: ix + iw * 0.42, y: iy, w: iw * 0.56, h: ih, fontFace: F, fontSize: TS.cardBody, color: C.body, valign: "middle", margin: 0, lineSpacingMultiple: 1.14 });
  });
  return y + h;
}
...
function matrix(s, x, y, w, h, axis, cells, hi) {
  ...
  cells.forEach((c, i) => {
    ...
    if (on) s.addShape("rect", { x: cx, y: cy, w: cw, h: ch, fill: { color: C.wash } });
    s.addText(c.title, { x: cx + 0.18, y: cy + 0.16, w: cw - 0.36, h: 0.34, fontFace: F, fontSize: TS.cardTitle - 0.5, bold: true, color: on ? C.accent : C.ink, margin: 0, valign: "middle" });
    if (c.note) s.addText(c.note, { x: cx + 0.18, y: cy + 0.54, w: cw - 0.36, h: ch - 0.68, fontFace: F, fontSize: TS.cardBody, color: C.body, margin: 0, valign: "top", lineSpacingMultiple: 1.16 });
  });
  s.addShape("line", { x: gx, y: y + ch, w: gw, h: 0, line: { color: C.rule, width: 1 } });
  s.addShape("line", { x: gx + cw, y, w: 0, h, line: { color: C.rule, width: 1 } });
  s.addShape("rect", { x: gx, y: y + h, w: gw, h: 0.02, fill: { color: C.ink } });
  s.addShape("rect", { x: gx, y, w: 0.02, h, fill: { color: C.ink } });

  // 가로축
  s.addText(axis.x[0], { x: gx, y: y + h + 0.08, w: cw, h: 0.28, fontFace: F, fontSize: TS.axis, color: C.mute, margin: 0 });
  s.addText(axis.x[1], { x: gx + cw, y: y + h + 0.08, w: cw - 0.04, h: 0.28, fontFace: F, fontSize: TS.axis, color: C.mute, align: "right", margin: 0 });
  if (axis.xName) s.addText(axis.xName, { x: gx, y: y + h + 0.08, w: gw, h: 0.28, fontFace: F, fontSize: TS.axis, bold: true, color: C.ink, align: "center", margin: 0 });

  // 세로축 — 위아래 방향을 말로 적고 축 이름은 세워서 넣는다
  if (axis.y) {
    s.addText(axis.y[1], { x: x, y: y - 0.02, w: gut - 0.10, h: 0.26, fontFace: F, fontSize: TS.axis, color: C.mute, align: "right", valign: "middle", margin: 0 });
    s.addText(axis.y[0], { x: x, y: y + h - 0.24, w: gut - 0.10, h: 0.26, fontFace: F, fontSize: TS.axis, color: C.mute, align: "right", valign: "middle", margin: 0 });
    if (axis.yName) {
      s.addText(axis.yName, { x: x + gut / 2 - h / 2, y: y + h / 2 - 0.15, w: h, h: 0.30, rotate: 270,
...
function timeline(s, x, y, w, nodes, opts = {}) {
  ...
  s.addShape("rect", { x, y: base, w, h: 0.02, fill: { color: C.rule } });
  nodes.forEach((nd, i) => {
    const cx = x + i * slot + slot / 2, on = i === opts.hi;
    s.addText(nd.when, { x: cx - slot / 2 + 0.08, y, w: slot - 0.16, h: 0.34, fontFace: F, fontSize: TS.cardTitle - 0.5, bold: true, color: on ? C.accent : C.ink, align: "center", valign: "middle", margin: 0 });
    if (nd.what) s.addText(nd.what, { x: cx - slot / 2 + 0.08, y: y + 0.34, w: slot - 0.16, h: 0.28, fontFace: F, fontSize: TS.axis, color: C.mute, align: "center", valign: "middle", margin: 0 });
    s.addShape("ellipse", { x: cx - 0.085, y: base - 0.075, w: 0.17, h: 0.17, fill: { color: on ? C.accent : C.card }, line: { color: on ? C.accent : C.rule, width: 1.25 } });
    if (nd.note) s.addText(nd.note, { x: cx - slot / 2 + 0.08, y: base + 0.20, w: slot - 0.16, h: noteH, fontFace: F, fontSize: TS.cardBody, color: C.body, align: "center", valign: "top", margin: 0, lineSpacingMultiple: 1.16 });
  });
...
function compare(s, x, y, w, heads, rows, hi, opts = {}) {
  ...
  [[x, lw, heads.left, hi === "left"], [rx, lw, heads.right, hi === "right"]].forEach(([bx, bw, t, on]) => {
    ...
    s.addText(t, { x: bx + 0.08, y, w: bw - 0.16, h: hh, fontFace: F, fontSize: TS.cardTitle, bold: true, color: on ? C.accent : C.ink, align: "center", valign: "middle", margin: 0 });
  });
  rows.forEach((r, i) => {
    const ry = y + hh + 0.06 + i * rh;
    s.addText(r.left, { x: x + 0.08, y: ry, w: lw - 0.16, h: rh, fontFace: F, fontSize: TS.cardBody, color: hi === "left" ? C.ink : C.body, align: "center", valign: "middle", margin: 0 });
    s.addText(r.label, { x: mx, y: ry, w: mw, h: rh, fontFace: F, fontSize: TS.cardBody, bold: true, color: C.mute, align: "center", valign: "middle", margin: 0 });
    s.addText(r.right, { x: rx + 0.08, y: ry, w: lw - 0.16, h: rh, fontFace: F, fontSize: TS.cardBody, color: hi === "right" ? C.ink : C.body, align: "center", valign: "middle", margin: 0 });
    if (i < rows.length - 1) s.addShape("rect", { x, y: ry + rh, w, h: 0.008, fill: { color: C.rule } });
  });
...
function tree(s, x, y, w, root, children) {
  ...
  s.addText(root.label, { x: rx + 0.08, y, w: rw - 0.16, h: rh, fontFace: F, fontSize: TS.cardTitle - 0.5, bold: true, color: C.accent, align: "center", valign: "middle", margin: 0 });

  const n = children.length, slot = w / n, ty = y + rh + 0.34, ch = 0.92;
  s.addShape("line", { x: x + w / 2, y: y + rh, w: 0, h: 0.16, line: { color: C.rule, width: 1 } });
  s.addShape("line", { x: x + slot / 2, y: y + rh + 0.16, w: w - slot, h: 0, line: { color: C.rule, width: 1 } });
  children.forEach((c, i) => {
    const cx = x + i * slot, bw = slot - 0.20;
    s.addShape("line", { x: cx + slot / 2, y: y + rh + 0.16, w: 0, h: 0.18, line: { color: C.rule, width: 1 } });
    box(s, { x: cx + 0.10, y: ty, w: bw, h: ch, fill: { color: C.card }, line: { color: C.rule, width: 0.75 } });
    s.addText(c.label, { x: cx + 0.16, y: ty + 0.08, w: bw - 0.12, h: 0.32, fontFace: F, fontSize: TS.layer, bold: true, color: C.ink, align: "center", valign: "middle", margin: 0 });
    if (c.note) s.addText(c.note, { x: cx + 0.16, y: ty + 0.42, w: bw - 0.12, h: ch - 0.50, fontFace: F, fontSize: TS.cardBody, color: C.body, align: "center", valign: "top", margin: 0, lineSpacingMultiple: 1.14 });
  });
  return ty + ch;
}
...
function waterfall(s, x, y, w, h, items, opts = {}) {
  ...
  seq.forEach((q, i) => {
    ...
    s.addShape("rect", { x: cx, y: top, w: bw, h: Math.max(bot - top, 0.02), fill: { color: fill } });
    const lab = isEnd ? fmtNum(q.value) : (q.value >= 0 ? "+" : "-") + fmtNum(Math.abs(q.value));
    s.addText(lab, { x: cx - 0.30, y: top - 0.24, w: bw + 0.60, h: 0.22, fontFace: F, fontSize: TS.value,
      bold: isEnd, color: isEnd ? C.ink : C.body, align: "center", margin: 0 });
    s.addText(q.label, { x: x + i * slot, y: y + plotH + 0.10, w: slot, h: 0.40, fontFace: F, fontSize: TS.axis,
      bold: isEnd, color: isEnd ? C.ink : C.mute, align: "center", valign: "top", margin: 0, lineSpacingMultiple: 1.02 });
    if (i < n - 1 && seq[i + 1].type === "delta") {
      s.addShape("line", { x: cx + bw, y: yOf(q.to), w: slot - bw, h: 0, line: { color: C.rule, width: 0.75, dashType: "dash" } });
    }
  });
...
function lineTrend(s, x, y, w, h, labels, series, opts = {}) {
  ...
  if (m > 1) {
    let lx = x + 0.04;
    series.forEach((sr, si) => {
      s.addShape("rect", { x: lx, y: y + 0.12, w: 0.22, h: 0.06, fill: { color: C.series[si] } });
      s.addText(sr.name, { x: lx + 0.28, y: y, w: 1.22, h: 0.26, fontFace: F, fontSize: TS.legend, color: C.body, margin: 0, valign: "middle" });
      lx += 1.28;
    });
  }
  series.forEach((sr, si) => {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      s.addShape("line", { x: x1, y: y1, w: x2 - x1, h: y2 - y1, line: { color: col, width: 1.75 } });
    }
    sr.vals.forEach((v, i) => {
      s.addShape("ellipse", { x: xOf(i) - 0.065, y: yOf(v) - 0.065, w: 0.13, h: 0.13, fill: { color: col } });
      if (opts.values !== false && (opts.values === true || si === 0 || m === 1)) {
        s.addText(fmtNum(v, opts.digits), { x: xOf(i) - 0.48, y: yOf(v) - 0.34, w: 0.96, h: 0.20,
          fontFace: F, fontSize: TS.value, color: C.body, align: "center", margin: 0 });
      }
    });
  });
  labels.forEach((lb, i) => {
    s.addText(lb, { x: xOf(i) - 0.56, y: plotY + plotH + 0.08, w: 1.12, h: 0.30, fontFace: F, fontSize: TS.axis,
...
function stackedBar(s, x, y, w, rows, opts = {}) {
  ...
  if (opts.legend !== false) {
    let lx = x + lw;
    names.forEach((nm, si) => {
      s.addShape("rect", { x: lx, y: y + 0.09, w: 0.20, h: 0.11, fill: { color: C.series[si] } });
      s.addText(nm, { x: lx + 0.26, y: y, w: 1.24, h: 0.28, fontFace: F, fontSize: TS.legend, color: C.body, margin: 0, valign: "middle" });
      lx += 1.34;
    });
    ly = y + 0.40;
  }
  rows.forEach((r, ri) => {
    ...
    s.addText(r.label, { x, y: by, w: lw - 0.12, h: rh, fontFace: F, fontSize: TS.layer - 1, bold: true, color: C.ink, valign: "middle", margin: 0 });
    let cx = bx;
    r.parts.forEach((p, si) => {
      const pw = p.value / tot * bw, col = C.series[si];
      s.addShape("rect", { x: cx, y: by, w: pw, h: rh, fill: { color: col } });
      if (pw > 0.52) {
        s.addText((p.value / tot * 100).toFixed(1) + "%", { x: cx, y: by, w: pw, h: rh, fontFace: F, fontSize: TS.axis,
...
function chevron(s, x, y, w, h, steps, opts = {}) {
  ...
  steps.forEach((st, i) => {
    ...
    s.addShape("chevron", { x: bx, y, w: bw, h, fill: { color: on ? C.wash : C.card },
      line: { color: on ? C.accent : C.rule, width: on ? 1.25 : 0.75 } });
    s.addText(st.label, { x: bx + 0.24, y: st.note ? y + 0.08 : y, w: bw - 0.44, h: st.note ? 0.32 : h,
      fontFace: F, fontSize: TS.layer, bold: true, color: on ? C.accent : C.ink, align: "center", valign: "middle", margin: 0 });
    if (st.note) s.addText(st.note, { x: bx + 0.24, y: y + 0.42, w: bw - 0.44, h: h - 0.50, fontFace: F, fontSize: TS.axis,
...
function swimlane(s, x, y, w, cols, lanes, opts = {}) {
  ...
  cols.forEach((c, i) => {
    s.addText(c, { x: x + lw + i * cw, y, w: cw, h: hh, fontFace: F, fontSize: TS.gridHead, bold: true,
      color: C.ink, align: "center", valign: "middle", margin: 0 });
  });
  ruleThin(s, y + hh, x, w);
  lanes.forEach((L, j) => {
    const ly = y + hh + 0.06 + j * rh;
    s.addText(L.name, { x, y: ly, w: lw - 0.10, h: rh, fontFace: F, fontSize: TS.gridCell + 0.5, bold: true,
    ...
    L.cells.forEach((t, i) => {
      ...
      box(s, { x: cx, y: ly + 0.06, w: bw, h: rh - 0.16, fill: { color: on ? C.wash : C.card },
        line: { color: on ? C.accent : C.rule, width: on ? 1.25 : 0.75 } });
      s.addText(t, { x: cx + 0.06, y: ly + 0.06, w: bw - 0.12, h: rh - 0.16, fontFace: F, fontSize: TS.gridCell,
        color: on ? C.accent : C.body, align: "center", valign: "middle", margin: 0, lineSpacingMultiple: 1.08 });
      const nxt = L.cells[i + 1];
      if (nxt) s.addShape("rightArrow", { x: cx + bw + 0.005, y: ly + rh / 2 - 0.09, w: 0.13, h: 0.16, fill: { color: C.rule } });
    });
...
function funnel(s, x, y, w, stages, opts = {}) {
  ...
  stages.forEach((st, i) => {
    ...
    s.addText(st.label, { x: bx, y: by, w: bw, h: rh, fontFace: F, fontSize: TS.cardBody, bold: true,
      color: textOn(col), align: "center", valign: "middle", margin: 0 });
    s.addText(fmtNum(st.value) + (opts.unit || ""), { x: x + zone + 0.16, y: by, w: 1.16, h: rh,
    ...
    if (i > 0) {
      ...
      s.addText("전 단계 대비 " + conv.toFixed(0) + "%" + (st.note ? " · " + st.note : ""),
        { x: x + zone + 1.60, y: by, w: rest, h: rh, fontFace: F, fontSize: TS.axis, color: C.body, valign: "middle", margin: 0 });
    } else if (st.note) {
      s.addText(st.note, { x: x + zone + 1.60, y: by, w: rest, h: rh, fontFace: F, fontSize: TS.axis, color: C.mute, valign: "middle", margin: 0 });
    }
  });
...
function heat(s, x, y, w, cols, rows, opts = {}) {
  ...
  s.addTable(trows, { x: x, y: y, w: w, colW: [lw].concat(cols.map(() => cw)),
...
function gantt(s, x, y, w, periods, tasks, opts = {}) {
  ...
  periods.forEach((p, i) => {
    s.addText(p, { x: x + lw + i * pw, y, w: pw, h: hh, fontFace: F, fontSize: TS.gridHead, bold: true,
      color: C.ink, align: "center", valign: "middle", margin: 0 });
  });
  ruleThin(s, y + hh, x, w);
  const bodyH = tasks.length * rh;
  for (let i = 1; i < periods.length; i++) {
    s.addShape("line", { x: x + lw + i * pw, y: y + hh, w: 0, h: bodyH + 0.06, line: { color: C.rule, width: 0.75 } });
  }
  tasks.forEach((t, j) => {
    const ry = y + hh + 0.06 + j * rh, on = opts.hi === j;
    s.addText(t.label, { x, y: ry, w: lw - 0.10, h: rh, fontFace: F, fontSize: TS.gridCell + 0.5, color: C.ink, valign: "middle", margin: 0 });
    ...
    if (t.note) s.addText(t.note, { x: bx, y: ry + 0.09, w: bw, h: rh - 0.24, fontFace: F, fontSize: TS.axis,
