...
- [ ] TO:BUILDER FROM:REVIEW (D-20260905-09) **다섯 장을 겹쳐 보니 7건이 나왔다.**
      한 장씩 볼 때는 안 보이던 것들이다 (개별 검토는 다섯 다 0건이었다).
      결과: `~/deck-qa-jobs/family_review_20260905.json` (잡에 속하지 않아 잡 폴더 밖)
      - MAJOR 004·005 제목이 글자 하나 다르지 않은데 서로 없는 정보를 갖고 있다
      - MAJOR 결론의 자리와 그릇이 다섯 장에서 네 가지다
      - MAJOR 소제목 오른쪽 회색 자리가 007에서만 다른 일을 한다
      - MAJOR 006만 다른 양식인데 왜 다른지가 장표에 표시되지 않는다
      - MAJOR 배너 둘째 줄 글자색이 003만 금색, 나머지는 옅은 청색
      - MINOR 옅은 청색이 다섯 장에 걸쳐 세 가지를 가리킨다
      - MINOR 제목이 소재만 말하는 장이 셋이다
      처리 방침을 정해야 한다. 양식 규칙(house-rules)으로 굳힐 것과
      장표별 판단으로 남길 것이 섞여 있다.
