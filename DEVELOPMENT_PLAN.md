...
- `shin-ppt1`의 자체 검사기 `preflight.py`와 `audit.py`의 관계 (2.17).
  둘 다 파워포인트 구조를 본다. 겹치면 판정이 갈라진다 (2.14가 막으려는 상태).
  흡수할지, 그대로 부르고 결과만 합칠지 정해야 한다.
  `preflight.py`가 무엇을 검사하는지 먼저 읽고 판단한다.
