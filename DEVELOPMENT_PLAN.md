...
허용 글꼴과 표 열 너비 합. 확인해 보니 **지금은 같은 판정을 낸다.**
열 너비 허용 오차가 양쪽 다 10000 EMU다.
...
1. **`preflight.py`를 그대로 쓴다.** 다시 만들지 않는다. 파일 구조 검사는 audit이 안 하는 일이고
   스킬 저자가 이미 만들어 둔 것이다.
2. **겹치는 둘은 `audit.py`가 정본이다.** `house-rules.yaml`을 읽기 때문이다.
   `preflight.py`의 그 두 검사는 이중 확인으로 남기되, 판정이 갈리면 `audit.py`를 따른다.
...
무엇을 할지 모르던 시점에 정한 값이다. 지금 554 < 617이라 아직 여유가 있다.
