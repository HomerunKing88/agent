...
**완료 (2026-09-05).** 결함 05가 잡힌다. 상자 14.4pt에 글자가 189.8pt로 그려지는 것을
`render.text_overflow`가 실측으로 낸다.

집 PC 조건은 바뀌었다. Windows는 그대로 PowerPoint COM을 쓰고, **그 밖의 환경은
LibreOffice headless의 SVG 경계를 쓴다** (CODEX, 271471a). 글자가 상자에 들어가면
경계가 도형 틀과 같고 안 들어가면 커지는 성질을 이용한다 — 잡 004의 도형 22개를
전부 대조해 확인했다. 덕분에 맥에서도 LAYOUT이 실제로 돈다.

폰트가 없거나 대체가 감지된 도형은 PASS가 아니라 SKIP으로 빠진다. 잰 도형이
하나도 없으면 결과 자체가 SKIP이다 (2.16-7). HY헤드라인M이 맥에 없어 제목은 늘 SKIP이다.

**줄바꿈 검사는 뺐다** (fb0ac03). 자동 줄바꿈은 텍스트 상자의 정상 동작이라
의도한 것과 넘쳐서 생긴 것을 구분하지 못했고, 진짜 넘침은 위 검사가 이미 잡는다.
...
작성 2026-08-29. **완료 2026-09-05** — 실전 잡 다섯(003·004·005·006·007)을 끝까지
돌렸고 다섯 다 검토 지적 0건, 아홉 게이트 전부 PASS, SKIP 없음이다. 사용자가 판단했다.

프롬프트는 `prompts/EDITOR.md`가 아니라 **`prompts/REVIEW.md`**다. 2026-08-30에
EDITOR와 CRITIC을 하나로 합쳤다 — 렌즈를 나누려고 사람을 둘로 늘렸더니 창·승인·왕복이
두 배가 됐다. 한 사람이 보되 렌즈 둘(CONTENT·DESIGN)을 각각 보고하고,
`review_lens_cover`가 그 이행을 검사한다. `schemas/editor.py`는 그대로 응답 검증이다.
