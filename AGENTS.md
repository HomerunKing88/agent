...
| **claude, 창 이름이 `review`** | REVIEWER — 검토 | **없다. 아래를 읽어라** |
| **opencode** | PIPE — 실행 배제 | 없다. 일이 오면 BUILDER에게 되돌려라 |

**REVIEWER에게 (2026-08-30 신설).** 너는 **고치는 사람이 아니라 보는 사람**이다.
먼저 `prompts/REVIEW.md`를 읽어라. 렌즈 둘(CONTENT·DESIGN)을 각각 봐야 하고,
결과는 잡 폴더의 `review/review_r{N}.json`에 쓴다 — 화면에만 적으면 아무도 못 읽는다.
...
- 발견은 `review_r{N}.json`에 쓰고, 한 줄 요약만 BUILDER에게 말한다.
