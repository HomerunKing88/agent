...
- **VERIFY(claude) = 검증.** **담당 파일이 없다.** 남의 결과를 깨 보는 역할이다.
  고치지 않고 찾아서 알린다. 발견은 `HANDOFF.md`에 적는다.
  파일을 안 가지므로 커밋 충돌이 원천적으로 없다. 고칠 것이 나오면 소유자에게 간다.
- **PIPE(opencode) = 실행 배제.** 일을 주지 않는다. 창은 참고용으로 남는다.

- 사용자는 폰에서 나에게만 지시한다. CODEX·VERIFY에게는 `./ask.sh <대상> "지시"`로 시킨다.
